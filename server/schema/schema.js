  ...
  fields: () => ({
    ...
    homes: {
      ...
      resolve(parentValue) {
        ...
        return (
          axios
            .get(`http://localhost:8000/homes/?agencyId=${parentValue.id}`)
...
const HomeType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    agencyId: {
      ...
      // main resolve function that returns the requested graphQL data.
      resolve(parentValue) {
        return axios.get(`http://localhost:8000/agency/${parentValue.agencyId}/`).then(res => res.data);
      }
    },
  ...
  fields: () => ({
    ...
    home: {
      ...
      resolve(parentValue, args) {
        return axios.get(`http://localhost:8000/homes/${args.id}/`).then(res => res.data);
      }
    },
    ...
    agency: {
      ...
      args: { id: { type: GraphQLInt } },
      resolve(parentValue, args) {
        return axios.get(`http://localhost:8000/agency/${args.id}/`).then(res => res.data);
      }
    },
    homes: {
      type: new GraphQLList(HomeType),
      resolve() {
        return axios.get("http://localhost:8000/homes/").then(res => res.data);
      }
    }
  })
});
